...
import time
import torchmetrics
from torchmetrics import AUROC as ROC
from torchmetrics import ROC as ROCC
...
class MAC_Unit(nn.Module):
    ...
    def pack_kernel(self, pack_Gab=True):
        ...
        for i in range(self.parallel_num):
            ...
            theta = theta.view(1, 2, 3).repeat(16, 1, 1)
            grid = F.affine_grid(theta, torch.Size([16, 1, 9, 9]),align_corners=True)
            transformed_tensor = F.grid_sample(new_kernel, grid,align_corners=True)
            all_kernels.append(transformed_tensor)
        return all_kernels
...
def fit(model, epochs, loss_func, mask_loss, lambda_, opt, trainset, testset, folder, testimg, warm_up=False, l2_norm=True, resume=False):
    lr_schedular = torch.optim.lr_scheduler.ReduceLROnPlateau(opt, mode='min', factor=0.5, min_lr=0.00001, patience=5, cooldown=5, verbose=True)
    loss_recorder = 1
    if resume:
    ...
    for epoch in range(len(loss_log), epochs):
        ...
        with torch.no_grad():
            f1_meter = F1Score(task='binary')
            predictionsc = []
            targetsc = []
            for xb, yb in testset:
                y_pred = model(xb, l2_norm)[0]
                predictionsc.extend(y_pred.detach().cpu().numpy())
                targetsc.extend(yb.detach().cpu().numpy())
            f1_meter.update(torch.tensor(predictionsc), torch.tensor(targetsc).long())
            f1 = f1_meter.compute()

            roc_meter = ROCC(task='binary')
            predictionss = []
            targetss = []
            for xb, yb in testset:
                y_pred = model(xb, l2_norm)[0]
                predictionss.extend(y_pred.detach().cpu().numpy())
                targetss.extend(yb.detach().cpu().numpy())
            roc_meter.update(torch.tensor(predictionss), torch.tensor(targetss).long())
            roc = roc_meter.compute()
            sensitivity_mean = torch.mean(roc[0]).item()
            specificity_meann = torch.mean(roc[1]).item()
            specificity_mean = (1 - specificity_meann)

            auc_meter = ROC(task='binary')
            predictions = []
            targets = []
            for xb, yb in testset:
                y_pred = model(xb, l2_norm)[0]
            ...
            auc_meter.update(torch.tensor(predictions), torch.tensor(targets).long())
            auc = auc_meter.compute()
            auc_mean = torch.mean(auc).item()
            # average_auc = torch.mean(auc)
            if epoch%5 == 0:
                os.mkdir(folder+"/ep"+str(epoch))
            ...
            print('sensitivity', sensitivity_mean)
            print('specificity', specificity_mean)
            print('AUC: ',auc_mean)
